...
{
    public abstract class BasicFiguresGL : GLU
    {
        // --- Fields ---
        ...
        private static void DrawBox(float size, uint type)
        {
            ...
            {
                OpenGL.glBegin(type);
                OpenGL.glNormal3f(n[i, 0], n[i, 1], n[i, 2]);
                OpenGL.glVertex3f(v[faces[i, 0], 0], v[faces[i, 0], 1], v[faces[i, 0], 2]);
                OpenGL.glVertex3f(v[faces[i, 1], 0], v[faces[i, 1], 1], v[faces[i, 1], 2]);
                OpenGL.glVertex3f(v[faces[i, 2], 0], v[faces[i, 2], 1], v[faces[i, 2], 2]);
                OpenGL.glVertex3f(v[faces[i, 3], 0], v[faces[i, 3], 1], v[faces[i, 3], 2]);
                OpenGL.glEnd();
            }
        }
        ...
        private static void Doughnut(float r, float R, int nsides, int rings)
        {
            ...
            for (int i = rings - 1; i >= 0; i--)
            {
                ...
                for (int j = nsides; j >= 0; j--)
                {
                    ...
                    OpenGL.glNormal3f(cosTheta1 * cosPhi, -sinTheta1 * cosPhi, sinPhi);
                    OpenGL.glVertex3f(cosTheta1 * dist, -sinTheta1 * dist, r * sinPhi);
                    OpenGL.glNormal3f(cosTheta * cosPhi, -sinTheta * cosPhi, sinPhi);
                    OpenGL.glVertex3f(cosTheta * dist, -sinTheta * dist, r * sinPhi);
                }
                OpenGL.glEnd();
                theta = theta1;
                cosTheta = cosTheta1;
        ...
        private static void Pentagon(int a, int b, int c, int d, int e, uint shadeType)
        {
            ...
            OpenGL.glBegin(shadeType);
            OpenGL.glNormal3f(n0[0], n0[1], n0[2]);
            OpenGL.glVertex3f(dodec[a, 0], dodec[a, 1], dodec[a, 2]);
            OpenGL.glVertex3f(dodec[b, 0], dodec[b, 1], dodec[b, 2]);
            OpenGL.glVertex3f(dodec[c, 0], dodec[c, 1], dodec[c, 2]);
            OpenGL.glVertex3f(dodec[d, 0], dodec[d, 1], dodec[d, 2]);
            OpenGL.glVertex3f(dodec[e, 0], dodec[e, 1], dodec[e, 2]);
            OpenGL.glEnd();
        }
        ...
        private static void RecordItem(float[] n1, float[] n2, float[] n3, uint shadeType)
        {
            ...
            OpenGL.glBegin(shadeType);
            OpenGL.glNormal3fv(q1);
            OpenGL.glVertex3fv(n1);
            OpenGL.glVertex3fv(n2);
            OpenGL.glVertex3fv(n3);
            OpenGL.glEnd();
        }
        ...
        private static void DrawTeapot(int grid, double scale, uint type)
        {
            ...
            OpenGL.glPushAttrib(OpenGL.GL_ENABLE_BIT | OpenGL.GL_EVAL_BIT);
            OpenGL.glEnable(OpenGL.GL_AUTO_NORMAL);
            OpenGL.glEnable(OpenGL.GL_NORMALIZE);
            OpenGL.glEnable(OpenGL.GL_MAP2_VERTEX_3);
            OpenGL.glEnable(OpenGL.GL_MAP2_TEXTURE_COORD_2);
            OpenGL.glPushMatrix();
            OpenGL.glRotatef(270.0f, 1.0f, 0.0f, 0.0f);
            OpenGL.glScalef(0.5f * (float)scale, 0.5f * (float)scale, 0.5f * (float)scale);
            OpenGL.glTranslatef(0.0f, 0.0f, -1.5f);
            for (i = 0; i < 10; i++)
            {
                for (j = 0; j < 4; j++)
                {
                ...
                OpenGL.glMap2f(OpenGL.GL_MAP2_TEXTURE_COORD_2, 0, 1, 2, 2, 0, 1, 4, 2, tex1);
                ...
                OpenGL.glMap2f(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, p1);
                OpenGL.glMapGrid2f(grid, 0.0f, 1.0f, grid, 0.0f, 1.0f);
                OpenGL.glEvalMesh2(type, 0, grid, 0, grid);
                ...
                OpenGL.glMap2f(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, q1);
                OpenGL.glEvalMesh2(type, 0, grid, 0, grid);
                if (i < 6)
                {
                    for (int d = 0; d < 4; d++)
                    ...
                    OpenGL.glMap2f(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, r1);
                    OpenGL.glEvalMesh2(type, 0, grid, 0, grid);
                    ...
                    OpenGL.glMap2f(OpenGL.GL_MAP2_VERTEX_3, 0, 1, 3, 4, 0, 1, 12, 4, s1);
                    OpenGL.glEvalMesh2(type, 0, grid, 0, grid);
                }
            }
            OpenGL.glPopMatrix();
